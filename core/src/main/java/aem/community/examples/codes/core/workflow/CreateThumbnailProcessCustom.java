package aem.community.examples.codes.core.workflow;
...
@Properties({
        @Property(name = Constants.SERVICE_DESCRIPTION,
                value = "THF Custom thumbnail creation Process"),
        @Property(name = Constants.SERVICE_VENDOR, value = "Wipro Technologies"),
        @Property(name = "process.label", value = "THF Custom thumbnail creation Process") })
public class CreateThumbnailProcessCustom implements WorkflowProcess
      {
   @Reference
   ...
   {
     PROCESS_ARGS("PROCESS_ARGS"),  DIMENSION("dimensions"),  QUALITY("quality"),  MIME_TYPE("mimetypes"),  KEEP_FORMAT_LIST("keepFormatList"),  SKIP("skip");
...
throws WorkflowException
 ...
 LoggerUtil.errorLog(this.getClass(), "Inside execute method and Payload is "
               + "::{}",payLoadPath);
 Resource resourceRes=adminSessionService.getAdministrativeResourceResolver()
 ...
 String processArgs = (String)metaDataMap.get(Arguments.PROCESS_ARGS.name(), 
               String.class);
 Config config = parseLegacyConfig(processArgs);  
 LoggerUtil.errorLog(this.getClass(), "Process Arguments, Dimesions ::{} and MimeTypes::{}"
          + " and Quality ::{}", config.dimensions,config.mimeTypes,config.quality);
 for(String mimeType:config.mimeTypes){
 for(String dimension:config.dimensions){
 LoggerUtil.errorLog(this.getClass(), "Inside loop, MimeTypes::{} and "
  + "Dimensions ::{} ", mimeType,dimension);
 DimensionsConfig dimensionConfig=parseDimensionsArguments(dimension);
 LoggerUtil.errorLog(this.getClass(), "Inside loop, Afert getting dimensions,Dimensions"
  + " - Width  ::{} and height ::{}",dimensionConfig.height ,dimensionConfig.width);
 try {
this.createWebEnabledImage(workItem, dimensionConfig, config, asset, mimeType, renditionMaker);
} catch (RepositoryException e) {
 LoggerUtil.errorLog(this.getClass(),"Exception occured in createWebEnabledImages::{}",e);
}
 }
 }
}
 
public void createWebEnabledImage(WorkItem workItem, DimensionsConfig dimensionConfig, Config config,
Asset asset, String mimeType,RenditionMaker renditionMaker)throws RepositoryException
 {
   if (handleAsset(asset, config))
   {
     asset.setBatchMode(true);      
     RenditionTemplate template = renditionMaker.createWebRenditionTemplate(asset, dimensionConfig.width,
 ...
 private Config parseLegacyConfig(String processArgs)
 {
   ...
   String qualityStrGif = null;
   
   String[] args = processArgs.split(",");
   
   cfg.dimensions = getFirstValueFromArgs(args, Arguments.DIMENSION.legacyName, null).split(";");

   cfg.mimeTypes=getFirstValueFromArgs(args, Arguments.MIME_TYPE.legacyName, "image/png").split(";");
   ...
   for(String mimeTyp:cfg.mimeTypes){
    if(mimeTyp.equalsIgnoreCase("image/gif")){
    qualityStrGif=mimeTyp;
    }
   }
   //qualityStrGif.equals("image/gif") ? "256" : "60"
   String qualityStr = getFirstValueFromArgs(args, Arguments.QUALITY.legacyName, 
                       "100");
   cfg.quality = Integer.valueOf(qualityStr).intValue();
   
   List<String> values = getValuesFromArgs(Arguments.SKIP.legacyName, args);
   cfg.skipMimeTypes = ((String[])values.toArray(new String[values.size()]));
 ...
 public static DimensionsConfig parseDimensionsArguments(String arg)
 {
   ...
   if (str.contains("["))
   {
     ...
     if (str == null)
     {
       LoggerUtil.warnLog(CreateThumbnailProcessCustom.class,"parseConfig: cannot parse width/height, "
        + "missing closing bracket '{}'.", arg);
       return null;
     }
   }
   ...
   if (fragments.length >= 2) {
     ...
     {
       LoggerUtil.warnLog(CreateThumbnailProcessCustom.class,"parseConfig: cannot parse,"
        + " invalid width/height specified in config '{}': ", str, e);
       return null;
     }
   }
   LoggerUtil.warnLog(CreateThumbnailProcessCustom.class,"parseConfig: cannot parse,"
    + " insufficient arguments in config '{}'.", str);
   return null;
 }
 ...
 protected boolean handleAsset(Asset asset, Config config)
 {
   ...
   for (String val : config.skipMimeTypes) {
     ...
     {
       LoggerUtil.warnLog(CreateThumbnailProcessCustom.class," skipped for MIME type: " + mimeType);
       return false;
     }
   }
   return true;
